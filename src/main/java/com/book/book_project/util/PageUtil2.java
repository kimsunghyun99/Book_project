package com.book.book_project.util;
...
public class PageUtil2 {
	...
	public String getPageList(int pageNum, int postNum, int pageListCount, int totalCount, String category) throws UnsupportedEncodingException {
		...
		String encodedCategory = URLEncoder.encode(category, StandardCharsets.UTF_8.toString());
			...
			if(totalPage != 1) {
				for(int i=1; i<=pageListCount; i++) {
					...
					// - i == 1
					if(section > 1 && i == 1)
						pageList += " <a href=productList?category=" + encodedCategory + "&page=" + Integer.toString((section-2)*pageListCount + pageListCount)
								+ ">◀</a>";
					// 2. 페이지 출력 중단
					...
					if(pageNum != (section-1)*pageListCount + i)
						pageList += " <a href=productList?category=" + encodedCategory + "&page=" + Integer.toString((section-1)*pageListCount + i)
					...
					if(i==pageListCount && totalSection > 1 && totalPage >= i+(section-1)*pageListCount+1)
						pageList += " <a href=productList?category=" + encodedCategory + "&page=" + Integer.toString(section*pageListCount+1) + ">▶</a>";
				}
			}
			return pageList;
		}
